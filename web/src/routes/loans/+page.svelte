  ...
  <div class="card list-card">
    {#each S.db.loans as l (l.loan_id)}
      <a class="row" href={`/loans/${l.loan_id}`}>
        <div class="cat-dot" style="background:{l.is_paid ? '#22c55e' : '#f59e0b'}"></div>
        <div class="row-body">
...
{#if showForm}
  <div class="overlay" onclick={() => (showForm = false)}>
    <div class="sheet" onclick={(e) => e.stopPropagation()}>
      <h2 class="title" style="margin-bottom:16px">{i18n.t('loans.newLoan')}</h2>
      <div class="form-field">
        <label>{i18n.t('loans.person')}</label>
      ...
      <div class="form-field">
        <label>{i18n.t('loans.description')}</label>
        <input bind:value={description} />
      </div>
      <div class="grid2">
        <div class="form-field">
          <label>{i18n.t('loans.principal')}</label>
          <input bind:value={amount} inputmode="decimal" />
        </div>
        <div class="form-field">
          <label>{i18n.t('loans.rate')}</label>
          <input bind:value={rate} inputmode="decimal" placeholder="0" />
        </div>
      </div>
      <div class="form-field">
        <label>{i18n.t('loans.interest')}</label>
        <select bind:value={interestType}>
      ...
      <div class="grid2">
        <div class="form-field">
          <label>{i18n.t('loans.installments')}</label>
          <input bind:value={months} inputmode="numeric" />
        </div>
        <div class="form-field">
          <label>{i18n.t('loans.frequency')}</label>
      ...
      <div class="grid2">
        <div class="form-field">
          <label>{i18n.t('loans.startDate')}</label>
          <input bind:value={startDate} placeholder={i18n.t('expenses.datePlaceholder')} />
        </div>
        <div class="form-field">
          <label>{i18n.t('loans.dueDate')}</label>
          <input bind:value={dueDate} placeholder={i18n.t('expenses.datePlaceholder')} />
        </div>
      </div>
      {#if err}
